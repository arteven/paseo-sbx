...
import { buildActionSummaries, loadActionsConfig } from "./actions.server";
...
export interface SbxListResult {
  sandboxes: SbxSandbox[];
  error: string | null;
}

// Shared by listSandboxesHandler and runActionHandler (which needs a sandbox's cwd to run its
// command against). Not wrapped around `plugin.handle(...)` itself — see the note below.
export async function fetchSbxSandboxes(): Promise<SbxListResult> {
  let stdout: string;
  try {
...
export async function listSandboxesHandler(_input: {}, { paseo }: PluginHandlerContext) {
  // Independent of sbx's own health, so it's loaded regardless of which branch below returns.
  const { actions, warning: actionsWarning } = loadActionsConfig();
  const actionSummaries = buildActionSummaries(actions);

  const { sandboxes, error } = await fetchSbxSandboxes();
  if (error) {
    return { sandboxes: [], error, reconcile: EMPTY_RECONCILE, actions: actionSummaries, actionsWarning };
  }
  ...
  return { sandboxes, error: null, reconcile, actions: actionSummaries, actionsWarning };
}
